...
async def add_friend(friend_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    stmt = select(friends_association).where(
        ((friends_association.c.user_id_1 == current_user.id) & (friends_association.c.user_id_2 == friend_id)) |
        ((friends_association.c.user_id_1 == friend_id) & (friends_association.c.user_id_2 == current_user.id))
    )
    ...
    await db.execute(friends_association.insert().values(user_id_1=current_user.id, user_id_2=friend_id))
    await db.execute(friends_association.insert().values(user_id_1=friend_id, user_id_2=current_user.id))
    await db.commit()
    
    return {"message": "Friend added successfully"}

@router.get("/friends", response_model=list[UserResponse])
async def list_friends(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Get all users who are friends with the current user
    stmt = select(User).join(
        friends_association, User.id == friends_association.c.user_id_2

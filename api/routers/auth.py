...
from core.security import verify_password, get_password_hash, create_access_token, create_verification_token, create_password_reset_token
from core.config import settings
from models.user import User
from schemas.user import UserCreate, UserResponse, Token, PasswordResetRequest, PasswordResetConfirm
...
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    new_user = User(name=user.name, email=user.email.lower(), hashed_password=hashed_password, is_verified=False)
    db.add(new_user)
    await db.commit()

...
from schemas.balance import SimplifiedDebtsResponse, Debt, SettlementCreate, SettlementResponse
...
async def get_simplified_debts(group_id: int = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not group_id:
        debts = [d for d in debts if d.debtor_id == current_user.id or d.creditor_id == current_user.id]
        
    return SimplifiedDebtsResponse(debts=debts)

@router.post("/settle", response_model=SettlementResponse)
async def settle_debt(settlement_in: SettlementCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    payee = (await db.execute(select(User).where(User.id == settlement_in.payee_id))).scalars().first()
    if not payee:

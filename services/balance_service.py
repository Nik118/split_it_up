...
from models.expense import Expense, ExpenseSplit, Settlement


async def get_user_balances(
    user_id: int, db: AsyncSession, group_id: int = None
) -> Dict[int, float]:
    """
    ...
    if group_id:
        ...
        expense_ids_query = select(ExpenseSplit.expense_id).where(
        ...
        for s in settlements:
            net_balances[
                s.payer_id
            ] += s.amount  # Payer's net balance increases (they paid)
            net_balances[
                s.payee_id
            ] -= s.amount  # Payee's net balance decreases (they received)

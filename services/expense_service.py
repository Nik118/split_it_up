...
def validate_and_calculate_splits(expense_in: ExpenseCreate):
    ...
    if abs(total_paid - expense_in.total_amount) > 0.01:
        raise HTTPException(
            status_code=400,
            detail="Total amount paid must equal the total expense amount",
        )
    ...
    if expense_in.split_method == SplitMethod.EQUAL:
        ...
        if abs(total_owed - expense_in.total_amount) > 0.01:
            raise HTTPException(
                status_code=400,
                detail="Total amount owed must equal the total expense amount",
            )
        ...
        if abs(total_percentage - 100.0) > 0.01:
            raise HTTPException(
                status_code=400, detail="Total percentage must equal 100%"
            )

        for split in expense_in.splits:
            calculated_amount = round(
                expense_in.total_amount * (split.percentage / 100.0), 2
            )
        ...
        total_shares = sum(
            split.shares for split in expense_in.splits if split.shares is not None
        )
        if total_shares <= 0:
            raise HTTPException(
                status_code=400, detail="Total shares must be greater than 0"
            )

        for split in expense_in.splits:
            calculated_amount = round(
                expense_in.total_amount * (split.shares / total_shares), 2
            )
            split.amount_owed = calculated_amount
            total_assigned += calculated_amount
    ...
    if difference != 0:
        expense_in.splits[0].amount_owed = round(
            expense_in.splits[0].amount_owed + difference, 2
        )
